...
<html>
<head>
    <title>Nurse Call - Log Panggilan</title>
    <style>
        @page {
            margin: 0;
        }
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h1>Nurse Call - Log Panggilan</h1>
            <p class="subtitle">
                @php
                    $filters = [];
        ...
        <table>
            <tr>
                <td style="border: none; padding: 2px 0;">
                    <span class="total-info">Total Data: {{ count($calls) }} panggilan</span>
                </td>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ \Carbon\Carbon::parse($call->timestamp)->timezone('Asia/Jakarta')->format('d M Y H:i:s') }}</td>
                    <td>{{ $call->bed->username ?? '-' }}</td>
                    <td>
                        @php
                            $catName = strtoupper($call->category->name ?? '');
                        ...
                        @endphp
                        <span class="badge" style="background-color: {{ $catColor }};">{{ $call->category->name ?? '-' }}</span>
                    </td>
                    <td>{{ $call->duration }}</td>
                    <td>{{ $call->record ?? '-' }}</td>
                </tr>
